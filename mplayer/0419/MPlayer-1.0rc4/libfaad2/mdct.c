...
extern int g_imdcthandle;
extern aac_dec_ctx_s g_decoder;

void faad_imdct(mdct_info *mdct, real_t *X_in, real_t *X_out)
{
	if ( g_imdcthandle >= 0 )
	{
		g_decoder.i32Size = mdct->N;
		g_decoder.inbuf = (int *)X_in;
		g_decoder.outbuf = (int *)X_out;
		ioctl( g_imdcthandle, AAC_IOCSDEC, &g_decoder );
	}
	else
	{
    uint16_t k;
