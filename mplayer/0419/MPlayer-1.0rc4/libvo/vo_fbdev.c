...
#ifdef CONFIG_VENDOR_OSD
extern S_OSDDEMO g_osddemo;
#endif

extern int vendor_color_type;

void _demo_flip_page(void);
void _demo_flip_page3(void);
extern void _demo_fill_sound(void);
...
///	mp_msg(MSGT_VO, MSGL_INFO, "### fb_preinit open fb_dev_fd ###\n");

	if ( vendor_color_type <= 0 )	// default case
	{
		mp_msg( MSGT_VO, MSGL_INFO, "### Video format is RGB565. ###\n" );
///		if ( ioctl( fb_dev_fd, VIDEO_FORMAT_CHANGE, DISPLAY_MODE_RGB565 ) < 0 )
///			mp_msg( MSGT_VO, MSGL_INFO, "### VIDEO_FORMAT_CHANGE RGB565 - FAILED ###\n" );
///			mp_msg( MSGT_VO, MSGL_INFO, "### VIDEO_FORMAT_CHANGE RGB565 - OK ###\n" );
	}
	else
	{
		if ( ioctl( fb_dev_fd, VIDEO_FORMAT_CHANGE, DISPLAY_MODE_YCBYCR ) < 0 )
		{
			mp_msg( MSGT_VO, MSGL_INFO, "### VIDEO_FORMAT_CHANGE YCBYCR - FAILED ###\n" );
			goto err_out;
		}
		else
			mp_msg( MSGT_VO, MSGL_INFO, "### VIDEO_FORMAT_CHANGE YCBYCR - OK ###\n" );
	}
	...
	g_vpe.lcm_width = fb_vinfo.xres;
	g_vpe.lcm_height = fb_vinfo.yres;
	ioctl(fb_dev_fd, IOCTL_LCD_GET_DMA_BASE, &g_vpe.vpostBuffer);
	g_vpe.lcm_bits_per_pixel = fb_vinfo.bits_per_pixel;
	g_vpe.lcm_bytes_per_pixel = fb_vinfo.bits_per_pixel / 8;

#ifdef CONFIG_VENDOR_OSD
	g_osddemo.vpost_fd = fb_dev_fd;
	g_osddemo.lcm_bytes_per_pixel = g_vpe.lcm_bytes_per_pixel;
#endif

	mp_msg(MSGT_VO, MSGL_INFO, "### fb_preinit open, width = %d, height = %d, fb_preinit = 0x%x, bits_per_pixel = %d ###\n", g_vpe.lcm_width, g_vpe.lcm_height, g_vpe.vpostBuffer, g_vpe.lcm_bits_per_pixel);
...
///	mp_msg(MSGT_VO, MSGL_INFO, "### fbdev config width = %d, height = %d, d_width = %d, d_height = %d ###\n", width, height, d_width, d_height);

	width = d_width = fb_vinfo.xres;
	height = d_height = fb_vinfo.yres;
...
///		mp_msg(MSGT_VO, MSGL_INFO, "### config vo_draw_alpha_rgb32 ###\n");
        break;
    case 24:
        draw_alpha_p = vo_draw_alpha_rgb24;
		mp_msg(MSGT_VO, MSGL_INFO, "### config vo_draw_alpha_rgb24 ###\n");
        break;
    case 16:
		if ( vendor_color_type <= 0 )	// default case
...
///		mp_msg(MSGT_VO, MSGL_INFO, "### config vo_draw_alpha_rgb16 ###\n");
        break;
    case 15:
        draw_alpha_p = vo_draw_alpha_rgb15;
		mp_msg(MSGT_VO, MSGL_INFO, "### config vo_draw_alpha_rgb15 ###\n");
        break;
    case 12:
        draw_alpha_p = vo_draw_alpha_rgb12;
		mp_msg(MSGT_VO, MSGL_INFO, "### config vo_draw_alpha_rgb12 ###\n");
        break;
    default:
        return 1;
    }
...
#ifdef CONFIG_VENDOR_OSD
	g_osddemo.fb_line_len = fb_line_len;
#endif
...
#endif
    {
...
#ifdef CONFIG_VENDOR_OSD
		g_osddemo.frame_buffer = frame_buffer;
		g_osddemo.fb_page = fb_page;
		g_osddemo.frame_buffer2 = frame_buffer + 1 * fb_yres * fb_line_len;
		g_osddemo.fb_yres = fb_yres;
#endif
		...
		g_vpe.center = (unsigned char *)g_vpe.vpostBuffer +
                 ( (out_width  - in_width)  / 2 ) * fb_pixel_size +
                 ( (out_height - in_height) / 2 ) * fb_line_len +
                 x_offset * fb_pixel_size + y_offset * fb_line_len +
                 fb_page * fb_yres * fb_line_len;
...
#ifdef CONFIG_VENDOR_OSD
		g_osddemo.center = center;
#endif

///		mp_msg(MSGT_VO, MSGL_INFO, "### config, fb_page = %d, frame_buffer = 0x%x, g_vpe.center = 0x%x ###\n", fb_page, (unsigned int)frame_buffer, (unsigned int)g_vpe.center);
        ...
        if (fs || vm) {
            ...
            if (vo_doublebuffering)
                clear_size <<= 1;
			ioctl( fb_dev_fd, IOCTL_FB_LOCK );
			ioctl(g_vpe.vpe_fd, VPE_WAIT_INTERRUPT, NULL);
			if ( vendor_color_type <= 0 )	// default case
				memset(frame_buffer, 0, clear_size);
			else
			{
				for ( y_offset = 0; y_offset < clear_size; y_offset++ )
				{
					frame_buffer[y_offset++] = 0x00;
					frame_buffer[y_offset] = 0x80;
				}
///				mp_msg(MSGT_VO, MSGL_INFO, "### y_offset = %d, clear_size = %d ###\n", y_offset, clear_size);
			}
			ioctl( fb_dev_fd, IOCTL_FB_LOCK_RESET );
        }
    }
...
{
return 0;
///	mp_msg(MSGT_VO, MSGL_INFO, "### draw_slice, w = %d, h = %d, fb_pixel_size = %d, fb_line_len = %d, stride[0] = %d, x = %d, y = %d ###\n", w, h, fb_pixel_size, fb_line_len, stride[0], x, y);
...
///	uint8_t *d;
	...
	if ( g_jpeg.temp_dptr )
	{
///		if ( g_jpeg.dparam.scaled_width == fb_xres )
		{
///			d = center + fb_line_len * y + fb_pixel_size * x;
///			memcpy_pic2((unsigned int *)d, (unsigned int *)g_jpeg.temp_dptr, 0, g_jpeg.dparam.scaled_height, fb_line_len, fb_line_len, 0);
			#if 1
			;
			#else
			...
			{
				mp_msg(MSGT_VO, MSGL_INFO, "### decode - Error: read data error!\n" );
				return 0;
			}
			#endif
		}
///		else
		{
///			d = center + fb_line_len * y + fb_pixel_size * x;
///			memcpy_pic2(d, g_jpeg.temp_dptr, g_jpeg.dparam.scaled_width * fb_pixel_size, g_jpeg.dparam.scaled_height, fb_line_len, g_jpeg.dparam.scaled_width * fb_pixel_size, 1);
		}
	}
	else
	{
///		while ( !g_vpe.wait_complete )
		{
///			_demo_fill_sound();
///			ioctl(g_vpe.vpe_fd, VPE_POLLING_INTERRUPT, &g_vpe.wait_complete);
///			_demo_fill_sound();
			;///ioctl(g_vpe.vpe_fd, VPE_WAIT_INTERRUPT, NULL);
		}
	}
...
static void flip_page(void)
{
    ...
    if (vidix_name)
        return;
#endif
	if ( !g_jpeg.temp_dptr )	return;
    if (!vo_doublebuffering)
        return;

///	mp_msg(MSGT_VO, MSGL_INFO, "### flip_page, next_page = %d, fb_page = %d, page_delta = %d ###\n", next_page, fb_page, page_delta);
	if ( g_jpeg.temp_dptr )
	{
		;
...
///			fb_vinfo.yoffset = fb_page * fb_yres;
	...
	g_vpe.center += page_delta * fb_yres * fb_line_len;
...
///    close(fb_tty_fd);
	...
	if ( fb_page == 0 )
	{
		memcpy( g_osddemo.frame_buffer, g_osddemo.frame_buffer2, fb_yres * fb_line_len );
	}
#endif
...
///			mp_msg( MSGT_VO, MSGL_INFO, "### VIDEO_FORMAT_CHANGE YCBYCR - OK ###\n" );
	...
	{
///		mp_msg( MSGT_VO, MSGL_INFO, "### uninit frame buffer, prepare change to RGB565 ###\n" );
	...
	if ( fb_page == 0 )
	{
		if ( g_osddemo.frame_buffer && g_osddemo.frame_buffer2 )
			memcpy( g_osddemo.frame_buffer, g_osddemo.frame_buffer2, fb_yres * fb_line_len );
	}

    close(fb_dev_fd);
	mp_msg(MSGT_VO, MSGL_INFO, "### uninit close fb_dev_fd ###\n");
    if (frame_buffer)
        munmap(frame_buffer, fb_size);
    g_osddemo.frame_buffer = g_osddemo.frame_buffer2 = frame_buffer = NULL;
#ifdef CONFIG_VIDIX
    if (vidix_name)
        vidix_term();
#endif
