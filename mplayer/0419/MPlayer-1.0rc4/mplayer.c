...
int vendor_scale_type = -1;
int vendor_color_type = -1;
int vendor_rotate_type = -1;
int vendor_imdct_type = 1;
int vendor_reorder_type = -1;
int vendor_dx = -1;
int vendor_dy = -1;
int vendor_framedrop = 0;
int vendor_framedrop1 = 0;
...
#else
	mp_msg(MSGT_CPLAYER,MSGL_INFO,"ID_LENGTH= %.2lf\n", demuxer_get_time_length(mpctx->demuxer));
#endif
...
void uninit_player(unsigned int mask){
  ...
  if(mask&INITIALIZED_INPUT){
	...
	current_module="uninit_vpe";
	_demo_uninit_vpe();
	current_module="uninit_vde";
	_demo_uninit_vde();
...
#ifdef CONFIG_VENDOR_OSD
	_demo_set_mplayer_status( EXIT_QUIT );
#endif
...
#ifdef CONFIG_VENDOR_OSD
	_demo_set_mplayer_status( EXIT_EOF );
#endif
...
#ifdef CONFIG_VENDOR_OSD
	_demo_set_mplayer_status( EXIT_ERROR );
#endif
...
#ifdef CONFIG_VENDOR_OSD
	_demo_set_mplayer_status( EXIT_NONE );
#endif
...
///	int sleep_time;
	...
	// this is where mplayer sleeps during audio-only playback
	// to avoid 100% CPU use
///	mp_msg(NULL,MSGL_INFO,"###### ao_data.outburst = %d, ao_data.bps = %d ###\n", ao_data.outburst, ao_data.bps);
///	sleep_time = (ao_data.outburst - bytes_to_write) * 1000 / ao_data.bps;
///	if (sleep_time < 10) sleep_time = 10; // limit to 100 wakeups per second
///	mp_msg(NULL,MSGL_INFO,"###### sleep_time = %d ###\n", sleep_time);
///	usec_sleep(sleep_time * 1000);
///	usec_sleep(sleep_time);
	...
	if (decode_audio(sh_audio, playsize) < 0) // EOF or error
	{
		...
		{
///			t11 = GetTimer() - t11;
///			mp_msg(NULL,MSGL_INFO,"### 1 time = %d ###\n", t11);
		    return 0;
		}
	    }
	}
		...
		audio_eof = 0;
	t = GetTimer() - t;
///	mp_msg(NULL,MSGL_INFO,"### decode_audio_time = %d ###\n", t);
	tt = t*0.000001f; audio_time_usage+=tt;
	if (playsize > sh_audio->a_out_buffer_len) {
	    playsize = sh_audio->a_out_buffer_len;
	    if (audio_eof)
...
static void pause_loop(void)
{
	...
	g_osddemo.vendor_state = VENDOR_STATE_PAUSING;
#endif
	...
	g_osddemo.vendor_state = VENDOR_STATE_PLAYING;
#endif
...
#ifdef CONFIG_VENDOR_OSD
	g_osddemo.vendor_osd_level = -1;
	g_osddemo.vendor_osd_shmid = -1;
	g_osddemo.vendor_osd_shmsize = -1;
	g_osddemo.vendor_osd_count = 1;
	g_osddemo.vendor_pindex = -1;
#endif
...
///	mp_msg(MSGT_CPLAYER,MSGL_INFO,"### g_osddemo.vendor_pindex = %d ###\n",g_osddemo.vendor_pindex);
...
#ifdef CONFIG_VENDOR_OSD
        g_osddemo.vendor_state = VENDOR_STATE_IDLING;
#endif
    ...
    if(mpctx->sh_video){
        ...
        if(!video_read_properties(mpctx->sh_video)) {
            ...
            if(!mpctx->sh_video->fps && !force_fps && !correct_pts){
                ...
                correct_pts = 0; /* FIXME avoid run to no FPS case */
                mpctx->sh_video->fps=30.0f; /* FIXME assign a default FPS */
                mpctx->sh_video->frametime=1.0f/mpctx->sh_video->fps; /* FIXME assign a default FPS */
            }
        }   
    }
...
main:
   ...
   if(verbose) term_osd = 0;
   {
       ...
       {
           while ( _demo_check_mplayer_status() )
           {
               usleep(10);
           }
       }
