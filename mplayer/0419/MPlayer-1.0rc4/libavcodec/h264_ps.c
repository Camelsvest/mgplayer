...
g_swidth = 16*sps->mb_width - 2*FFMIN(sps->crop_right, 7);
    if(sps->frame_mbs_only_flag)
        g_sheight= 16*sps->mb_height - 2*FFMIN(sps->crop_bottom, 7);
    else
        g_sheight= 16*sps->mb_height - 4*FFMIN(sps->crop_bottom, 3);
sps->mb_width = sps->mb_height = 1;
