...
#include "vendor/vdedef.h"
#include "vendor/DrvVDE.h"
#include "vendor/VpuApi.h"
...
static av_cold void common_init(H264Context *h){
	...
	av_log( s->avctx, AV_LOG_INFO, "### s->width = %d, s->height = %d ###\n", s->width, s->height );
    s->codec_id= s->avctx->codec->id;

	if ( s->width == 0 ) s_flag = 1;
	else s_flag = 1;
...
av_cold int ff_h264_decode_init(AVCodecContext *avctx){
	...
	av_log( avctx, AV_LOG_INFO, "### ff_h264_decode_init ###\n" );

	memset( (unsigned char *)&g_vde2, 0, sizeof(g_vde2) );
	g_vde2.needSeqInit = 1;
	g_vde2.ret = RETCODE_SUCCESS;
	...
	g_vpe.is_init = g_vpe.offset0 = g_vpe.current_rotate_type = g_vpe.current_scale_type = 0;
...
//    s->decode_mb= ff_h263_decode_mb;
    ...
    if(avctx->codec_id == CODEC_ID_H264){
		av_log( avctx, AV_LOG_INFO, "### CODEC_ID_H264 ###\n" );
    ...
    if(avctx->extradata_size > 0 && avctx->extradata && *(char *)avctx->extradata == 1){
		...
		av_log( avctx, AV_LOG_INFO,"### h->is_avc = %d ###\n", h->is_avc );
        ...
        for (i = 0; i < cnt; i++) {
            nalsize = AV_RB16(p) + 2;
			av_log( avctx, AV_LOG_INFO,"### Number of sps = %d, nalsize = %d ###\n", cnt, nalsize);
        ...
        for (i = 0; i < cnt; i++) {
            nalsize = AV_RB16(p) + 2;
			av_log( avctx, AV_LOG_INFO,"### Number of pps = %d, nalsize = %d ###\n", cnt, nalsize);
	...
	av_log( avctx, AV_LOG_INFO, "### ff_h264_decode_init done ###\n" );
...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size){
    MpegEncContext * const s = &h->s;
    AVCodecContext * const avctx= s->avctx;
    int buf_index=0, temp1;
    H264Context *hx; ///< thread context
    int context_count = 0;
    int next_avc= h->is_avc ? 0 : buf_size;
	float factor_w, factor_h;
	int temp_lcm_width = 0;
	int temp_lcm_height = 0;
      ...
      again:
        ...
        case NAL_SLICE:
            ...
            if(hx->redundant_pic_count==0 && hx->s.hurry_up < 5
				...
				*g_vde.pVBitStreamBuffer++ = 0x00;
				*g_vde.pVBitStreamBuffer++ = 0x00;
				*g_vde.pVBitStreamBuffer++ = 0x01;
				memcpy(g_vde.pVBitStreamBuffer, (unsigned char *)&buf[buf_index - consumed], consumed);
				g_vde.pVBitStreamBuffer+=consumed;
				g_vde2.decParam.chunkSize += (consumed + 3);
        ...
        case NAL_PPS:
			...
			*g_vde.pVBitStreamBuffer++ = 0x01;
			...
			if ( g_vde2.needSeqInit && s_flag )
			{
				g_vde2.needSeqInit = 0;
				if ( _demo_decoder_seqinit( avctx ) < 0 )
					return -1;
			}
	...
	if ( (hx->nal_unit_type == NAL_SLICE) || (hx->nal_unit_type == NAL_IDR_SLICE) )
	{
		if ( s_flag )
		{
...
///	av_log( avctx, AV_LOG_INFO, "### decode_frame ###\n" );
...
av_cold int ff_h264_decode_end(AVCodecContext *avctx)
{
	...
	av_log( avctx, AV_LOG_INFO, "### ff_h264_decode_end ###\n" );
