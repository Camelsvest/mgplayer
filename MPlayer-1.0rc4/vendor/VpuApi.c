...
PhysicalAddress paraBuffer,virtualparaBuf;

static RetCode VPU_InitPtrRegArray(void)
{
...
/*
RetCode VPU_EnableInterrupt(Uint32 IntSource)
{
	
	VpuWriteReg(BIT_INT_ENABLE, IntSource);
...
RetCode VPU_Init(PhysicalAddress workBuf, Uint32 virtualworkbuf)
{
	...
	virtualcodeBuffer = virtualworkbuf;
	virtualparaBuf = virtualworkbuf + CODE_BUF_SIZE + WORK_BUF_SIZE;// + PARA_BUF2_SIZE		
#endif	
	...
	for (i = 0; i < sizeof(bit_code) / sizeof(bit_code[0]); i += 2) 
	{
	   ...
	   VpuWriteMem(virtualcodeBuffer + i * 2, data);	   
	}
	...
	VpuWriteReg(BIT_INT_ENABLE, 0x4008); // PIC_RUN irq enable & Buffer Empty	
...
#endif
{
	...
	for (i = 0; i < num; ++i) {
		VpuWriteReg(virtualparaBuf + i * 3 * 4, bufArray[i].bufY);
		VpuWriteReg(virtualparaBuf + i * 3 * 4 + 4, bufArray[i].bufCb);
		VpuWriteReg(virtualparaBuf + i * 3 * 4 + 8, bufArray[i].bufCr);
	}
...
RetCode VPU_SleepWake(DecHandle handle, int iSleepWake)
{
	...
	if(iSleepWake==1) {
		for ( i = 0 ; i < 64 ; i++)
			regBk[i] = VpuReadReg(g_vde.BIT_BASE + 0x100 + (i * 4));
		...
		for ( i = 0 ; i < 64 ; i++)
			VpuWriteReg(g_vde.BIT_BASE + 0x100 + (i * 4), regBk[i]);
		VpuWriteReg(BIT_CODE_RUN,	   0);
		//---- LOAD BOOT CODE
		{	Uint32 data;
			for( i = 0; i <2048; i++ ) {
