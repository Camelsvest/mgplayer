...
#ifdef CONFIG_VENDOR_OSD
		case MP_CMD_VENDOR_OSD:{
                int v = cmd->args[0].v.i;
				g_osddemo.vendor_osd_level = v;
///				mp_msg(MSGT_CPLAYER, MSGL_INFO, "### g_osddemo.vendor_osd_level = %d ###\n", g_osddemo.vendor_osd_level);
            }
            break;

		case MP_CMD_GET_VENDOR_STATE:{
				_demo_set_vendor_state( g_osddemo.vendor_state );
			}
			break;

		case MP_CMD_VENDOR_SCALE:{
				int v = cmd->args[0].v.i;
				vendor_scale_type = v;
			}
			break;

		case MP_CMD_VENDOR_ROTATE:{
				int v = cmd->args[0].v.i;
				vendor_rotate_type = v;
			}
			break;

		case MP_CMD_VENDOR_DX:{
				int v = cmd->args[0].v.i;
				vendor_dx = v;
			}
			break;

		case MP_CMD_VENDOR_DY:{
				int v = cmd->args[0].v.i;
				vendor_dy = v;
			}
			break;
...
#ifdef CONFIG_VENDOR_OSD
				float time_length;
				if ( g_osddemo.vendor_state == VENDOR_STATE_IDLING )
					time_length = 0.0f;
				else
				{
					time_length = demuxer_get_time_length(mpctx->demuxer);
					mp_msg(MSGT_GLOBAL, MSGL_V, "ANS_LENGTH=%.2lf\n", time_length);
				}
				_demo_set_time_length( time_length );
#else
...
#ifdef CONFIG_VENDOR_OSD
				mp_msg(MSGT_GLOBAL, MSGL_V, "ANS_TIME_POSITION=%.1f\n", pos);
				_demo_set_sync_time( pos );
#else
